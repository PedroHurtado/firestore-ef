...
/// <summary>
/// Provides change tracking support for MapOf properties.
/// MapOf properties are ignored by EF Core (to prevent navigation detection),
/// so this helper uses shadow properties with JSON serialization to detect changes.
/// </summary>
public static class MapOfChangeTracker
{
    /// <summary>
    ...
    /// <summary>
    /// Synchronizes MapOf shadow properties and marks entities as Modified if changes are detected.
    /// Call this before base.SaveChanges() or base.SaveChangesAsync().
    /// </summary>
    /// <param name="context">The DbContext to synchronize.</param>
    public static void SyncMapOfChanges(DbContext context)
    {
        foreach (var entry in context.ChangeTracker.Entries())
        {
            SyncEntityMaps(entry);
        }
    }

    /// <summary>
    /// Synchronizes MapOf shadow properties for a single entity entry.
    /// </summary>
    internal static void SyncEntityMaps(EntityEntry entry)
    {
        ...
        foreach (var property in entityType.GetProperties())
        {
            ...
            var ignoredProps = entityType.GetMapOfIgnoredProperties(trackerFor);
            var currentJson = SerializeMapProperty(entry.Entity, trackerFor, ignoredProps);
    ...
    /// <summary>
    /// Initializes the shadow property with the current JSON value of the map.
    ...
    internal static void InitializeShadowProperties(EntityEntry entry, IEntityType entityType)
    {
        foreach (var property in entityType.GetProperties())
        {
            ...
            var ignoredProps = entityType.GetMapOfIgnoredProperties(trackerFor);
            var json = SerializeMapProperty(entry.Entity, trackerFor, ignoredProps);
            var shadowProp = entry.Property(property.Name);
        ...
        Microsoft.EntityFrameworkCore.ChangeTracking.Internal.InternalEntityEntry internalEntry)
    {
        foreach (var property in entityType.GetProperties())
        {
            ...
            var ignoredProps = entityType.GetMapOfIgnoredProperties(trackerFor);
            var json = SerializeMapProperty(entity, trackerFor, ignoredProps);
    ...
    /// <summary>
    /// Serializes a map property to JSON for comparison.
    /// </summary>
    /// <param name="entity">The entity containing the map property.</param>
    ...
    private static string? SerializeMapProperty(object entity, string propertyName, HashSet<string>? ignoredProperties)
    {
